...
// boutons :
#define BTN_RECORD  0    // Bouton enregistrement
#define BTN_PLAY    1    // Bouton lecture 
...
// CHAÎNE D'ENTRÉE : Micro → Passe-Haut → Passe-Bas → Record Queue
AudioConnection patchCord1(i2s_input, 1, filterHP, 0);        
AudioConnection patchCord2(filterHP, 0, filterLP, 0);         
AudioConnection patchCord3(filterLP, 0, recordQueue, 0);    

// CHAÎNE DE SORTIE : PlayQueue → Mixer → Casque
AudioConnection patchCord4(playQueue, 0, mixerLeft, 0);     
AudioConnection patchCord5(playQueue, 0, mixerRight, 0);      
AudioConnection patchCord6(mixerLeft, 0, i2s_output, 0);      
AudioConnection patchCord7(mixerRight, 0, i2s_output, 1);     
...
float compGain = 0.0;         // Gain actuel du compresseur
...
void loop() {
  // cooldown de 2 secondes entre enregistrements
  if (!canRecord && (millis() - lastRecordEndTime > 2000)) {
    canRecord = true;
    Serial.println("Prêt pour un nouvel enregistrement\n");
  }
...
void setupFilters() {
  // FILTRE PASSE-HAUT (100 Hz) - Élimine les basses fréquences
...
void stopRecording() {
  ...
  Serial.println("Attendre 2 secondes avant nouvel enregistrement...\n");
}
...
int16_t softClip(int16_t sample) {
  ...
  float clipped = tanh(normalized * 1.5) / 1.5; 
